...
import type { DuoWriteExercise, DuoLang } from "./types";
import { LessonProgressBar } from "./LessonProgressBar";
import { CheckAnswer } from "./CheckAnswer";

interface DuoWriteExerciseProps {
  problem: DuoWriteExercise;
  correctAnswerCount: number;
  totalCorrectAnswersNeeded: number;
  selectedAnswers: number[];
  setSelectedAnswers: (index: number) => void;
  ...
  hearts: number | null;
  onQuit: () => void;
  /** Glyph shown in the speech bubble (per game). */
  writeIcon: React.ReactNode;
  /** Label above the prompt, per language. */
  writeLabel: { es: string; en: string };
}

export const DuoWriteExerciseView: React.FC<DuoWriteExerciseProps> = ({
  problem,
  correctAnswerCount,
          ...
          <div className="w-full">
            ...
            <div className="flex min-h-[60px] flex-wrap gap-1 border-b-2 border-t-2 border-gray-200 py-1 mt-4">
              {selectedAnswers.map((i) => (
                ...
                onClick={() =>
                  !correctAnswerShown && setSelectedAnswers(i)
                }
                >
                  {answerTiles[i]}
                </button>
...
export default DuoWriteExerciseView;
